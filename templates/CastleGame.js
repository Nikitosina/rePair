class TFormCastleGame extends TControl {
	constructor(Canvas, room) {
		...
		this.on_castle_updated({'top_card': room['first_card']}); //creates this.top_card
...
//		this.ButtonUp1 = new TButton();
	...
	on_click(e) {
		if (!this.won) {
			var your_id_clicked = this.card.on_click(e);
			var top_id_clicked = this.top_card.on_click(e);
			var flag = false;
			...
			if (flag) {
				...
				socket.emit('update_castle', {'top_card': this.top_card});
...
class TCard {
	...
	Show() {
		//if (this.dy != 0) {this.dy += 1; this.y -= 1;}
	...
	on_click(e) {
		...
		if (x * x + y * y <= this.r * this.r) {
			for (var i of this.items) {
				...
				if (xi * xi + yi * yi <= (i.rc * i.scale) * (i.rc * i.scale)) {
					return i.number;
                }
			}
		}
